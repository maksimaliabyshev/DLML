...
HMODULE GetModuleHandleSimple(LPCSTR lpModuleName) {
	...
	if (Handle)
		...
		(void)MsgBoxExit(MB_ICONERROR, "Exiting", "Unable to create filesystem_x64_rwdi.dll handle");
	return Handle;
}
...
#pragma warning(suppress : 6387)
	...
	if (Address)
		...
		(void)MsgBoxExit(MB_ICONERROR, "Exiting", "Failed To Get Address");
	return Address;
}
...
void HookFunction(LPVOID target, LPVOID destination, LPVOID* original) {
	...
	if (status == MH_OK) {
		...
		(void)MsgBoxExit(MB_ICONERROR, "Exiting", "Failed to hook %p : %s", target, statusCode);
	}
}
